...
motor_rosidl_metadata_bytes="$(
  printf '%s\n' "$motor_rosidl_metadata_breakdown" |
    rosidl_breakdown_bytes __total)"
motor_rosidl_raw_source_bytes="$(
  printf '%s\n' "$motor_rosidl_metadata_breakdown" |
    rosidl_breakdown_bytes toplevel_type_raw_source)"
...
if [ -x "$ROOT/tools/motor-m2-telemetry-sweep.py" ]; then
  if motor_telemetry_sweep="$(cd "$ROOT" && \
      tools/motor-m2-telemetry-sweep.py --min-margin-pct 1 --pass-only 2>&1)"; then
...
motor_default_ram_bytes="$(size_brief_metric "$motor_default_size" ram_static_bytes)"
motor_opt_ram_bytes="$(size_brief_metric "$motor_opt_size" ram_static_bytes)"
...

{
  ...
  if [ "$motor_telemetry_sweep_status" = "ok" ] &&
    ...
    printf '%s\n' "$motor_telemetry_sweep" |
      ...
      '
    echo
    echo "- 说明：这里只展示 \`--min-margin-pct 1 --pass-only\` 摘要的前几行；完整矩阵请直接运行 \`tools/motor-m2-telemetry-sweep.py\`。"
    echo "- 运行顺序：\`state20_health200_2000000\` 是 first_smoke；\`921600\` 只能作为 2Mbps PASS 后的 post-2Mbps comparison only，且必须使用 low telemetry 候选，不能使用默认 \`state20_health200_921600\`。"
  else
  ...
  echo "- M2 ON 默认静态 RAM 当前约 ${motor_default_ram_bytes:-unknown}B，余量偏紧；\`build-motor-opt\` 的 stack/linker reserve 候选当前约 ${motor_opt_ram_bytes:-unknown}B，但必须等 motor-enabled 栈水位、MSP/heap、reconnect soak 证据后再改默认。"
  echo "- M2 motor 默认静态通信预算显示 200Hz target + 20ms state + 200ms health（50Hz/5Hz）在 921600 baud 超过 30% 预算，2Mbps 通过；应先测 \`2000000 first_smoke\`，只有它通过 Agent 建链、ROS graph、target→state、\`/com\` 并发后，才测 \`921600 post-2Mbps comparison only\`，且必须使用 low telemetry 候选。"
  ...
  if [ "${motor_rosidl_raw_source_bytes:-0}" -gt 0 ] 2>/dev/null; then
    echo "- M2 motor ELF 中 \`rosidl_type_metadata\` 当前约 ${motor_rosidl_metadata_bytes:-unknown}B RAM，其中 \`toplevel_type_raw_source\`=${motor_rosidl_raw_source_bytes}B，仍是内存优化重点；但 \`ROSIDL_TYPESUPPORT_SINGLE_TYPESUPPORT\` 曾是 T5 HardFault/agent 兼容修复的一部分，需用独立 libmicroros rebuild/strip type-description 矩阵验证后再改默认。"
  else
    echo "- M2 motor ELF 中 \`rosidl_type_metadata\` 当前约 ${motor_rosidl_metadata_bytes:-unknown}B RAM，\`toplevel_type_raw_source\`=0B；\`colcon.motor.notypedesc.meta\` 已消除 raw source 文本，下一步重点是验证 vanilla Agent/HardFault 兼容性并评估剩余 type metadata，不能只凭静态值改默认 libmicroros。"
  fi
  ...
  echo "1. 找不到 ST-LINK/USB-TTL 时先看 Windows：\`powershell.exe -NoProfile -Command \"usbipd list\"\`；若设备是 \`Shared\` 而非 \`Attached\`，先用 \`usbipd attach --wsl --busid <BUSID>\` attach 到 WSL。"
  echo "2. 再跑只读 SWD 诊断：\`tools/diagnose-swd.sh\`；若要自动化 gate，用 \`STRICT=1 tools/diagnose-swd.sh\`。"
  echo "3. 若 \`SWD_STATUS\` 不是 \`ok\`，按诊断输出先确认 Windows/WSL attach 状态，再检查线缆、供电、BOOT/RESET、SWDIO/SWCLK/NRST、usbip 独占和 ST-LINK 连接状态。"
  echo "4. SWD 恢复后先生成推荐阶梯命令：\`tools/recommend-staircase-command.sh\`，再执行其中的 \`tools/run-com-staircase.sh ...\` 和匹配的 \`tools/check-com-staircase-contract.py ...\`。"
  ...
  echo "5. M2 motor 真机首轮先生成推荐命令：\`tools/recommend-motor-m2-smoke-command.sh\`。默认使用 2Mbps 烧 \`EXO_MOTOR_ROS_ENTITIES=ON\` 固件；2Mbps first smoke PASS 后，若要跑 921600 post-2Mbps comparison only，先看 \`tools/motor-m2-telemetry-sweep.py --min-margin-pct 1 --pass-only\` 和完整矩阵，使用 low telemetry 候选而不是默认 20ms/200ms。启动 micro-ROS Agent，确认 \`/motor/tp_joint_target\`、\`/motor/tp_joint_state\`、\`/motor/tp_motor_health\` 和 \`/com/tp_mcu_status\` 同时可见，并把证据保存到 \`log/motor-m2-smoke/<tag>/\`。"
  echo "6. M2 motor topic 闭环按推荐命令执行：发空 \`frame_id\` 的 \`/motor/tp_joint_target\`，检查 \`JointState.last_target_seq\`；停止发布后检查 TTL stale；再注入非空 \`frame_id\` 确认被拒绝且 executor 不掉线；随后跑 200Hz enabled target soak，要求 \`targets_received\` 约 200Hz 增长、\`targets_applied\` 在 before/mid/after 持续增长、最终 \`last_target_seq\` 跟随最新 seq，且 \`/com/tp_mcu_status\` 在 soak 期间不掉线。"
  echo "7. M2 evidence 判定：确认 \`evidence.env\` 的硬件状态字段并设置 \`sample_only=false\` 后运行 \`tools/check-motor-m2-smoke-evidence.py log/motor-m2-smoke/<tag>\`；rate 与 stack 输出会自动解析，不要把只有 \`/com\` 的证据当作 M2 PASS。"
  echo "8. 高频 profile 跑通后读栈水位：\`tools/measure-stack-hwm.sh firmware/f103-microros/build/f103-microros.elf\`，motor-enabled 构建需额外记录 \`firmware/f103-microros/build-motor/f103-microros.elf\` 的 size report。"
  echo "9. 若 SWD 仍未恢复，继续 no-flash：\`BUILD_FIRMWARE=0 FLASH_FIRMWARE=0 tools/run-com-perf.sh noflash_\$(date +%H%M)\`。"
} >"$REPORT"
