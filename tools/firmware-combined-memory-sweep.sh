...
COMBINED_MEMORY_CASES="${COMBINED_MEMORY_CASES:-baseline:768:512:1024 stack704:704:512:1024 stack640:640:512:1024 linker_heap256_stack768:768:256:768 linker_heap0_stack768:768:0:768 linker_heap256_stack512:768:256:512 linker_heap0_stack512:768:0:512 stack704_heap256_stack768:704:256:768 stack704_heap0_stack768:704:0:768 stack704_heap256_stack512:704:256:512 stack704_heap0_stack512:704:0:512 stack640_heap0_stack512:640:0:512}"
...
echo "case,verdict,reason,microros_stack_words,microros_stack_bytes,newlib_heap_bytes,msp_stack_bytes,linker_user_heap_stack_bytes,control_loop_hz,control_timer_irq_priority,qos_best_effort,motor_ros_entities,status_every_n,uart_baud,executor_spin_timeout_us,uart_read_poll_yields,flash_bytes,flash_margin_bytes,ram_static_bytes,ram_static_margin_bytes,data_bytes,bss_bytes" >"$CSV"
{
  echo "| Case | verdict | reason | micro-ROS stack words | stack B | newlib heap B | MSP stack B | linker reserve B | loop Hz | timer IRQ prio | QoS best-effort | motor entities | status every | baud | spin timeout us | poll yields | Flash B | Flash margin B | static RAM B | RAM margin B | data B | bss B |"
  echo "|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"
} >"$MD"
...
EOF_CASE
  ...
  echo "[combined-memory-sweep] build case=$label stack_words=$stack_words heap=$heap_bytes msp_stack=$msp_stack_bytes loop_hz=$CONTROL_LOOP_HZ motor_entities=$MOTOR_ROS_ENTITIES"
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    ...
    "$MOTOR_ROS_ENTITIES" "$STATUS_EVERY_N" "$UART_BAUD" "$EXECUTOR_SPIN_TIMEOUT_US" \
