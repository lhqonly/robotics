...

{
  ...
  echo "4. M2 motor 真机首轮先生成推荐命令：\`tools/recommend-motor-m2-smoke-command.sh\`。默认使用 2Mbps 烧 \`EXO_MOTOR_ROS_ENTITIES=ON\` 固件，启动 micro-ROS Agent，确认 \`/motor/tp_joint_target\`、\`/motor/tp_joint_state\`、\`/motor/tp_motor_health\` 和 \`/com/tp_mcu_status\` 同时可见。"
