...
assert_contains "$summary" \
  ...
  "dry-run records case executor threads override for taskset case"
...
LOGDIR="$TMPDIR/scheduler_invalid" COM_PERF_LOGDIR="$TMPDIR/com-perf" DRY_RUN=1 \
  PC_SCHEDULER_CASES=$'bad_threads||many' \
  "$ROOT/tools/run-pc-scheduler-sweep.sh" dry_pc_sched_invalid >/dev/null

invalid_summary="$TMPDIR/scheduler_invalid/dry_pc_sched_invalid.summary.log"
assert_contains "$invalid_summary" \
  "reason=executor_threads_must_be_nonnegative_integer" \
  "invalid per-case executor threads are rejected"
