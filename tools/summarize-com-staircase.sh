...
awk -v format="$FORMAT" '
  ...
  function profile_verdict(stage, meta, target_rx_hz, seq_rate_hz,
      ...
      qos_incompatibility, expected_hz, min_ratio, max_ratio, max_p99_gap, max_max_gap,
    ...
    if (!is_missing(qos_incompatibility) && qos_incompatibility + 0 > 0) {
      verdict = "FAIL"
  ...
  BEGIN {
    ...
    if (format == "csv") {
      ...
      print "| Stage | verdict | reason | loop Hz | baud | timer IRQ prio | poll yields | spin us | PC Hz | QoS | status N | PC launch prefix | PC executor threads | status Hz | sampler Hz | target rx Hz | p95 gap s | p99 gap s | max gap s | zero gaps | seq Hz | seq delta avg/min/max | PC target Hz | PC gap p95/p99/max ms | PC catch-up events/extra | wire kbit/s | baud util % | lost | duplicate | inflight | QoS incompatible |"
      print "|---|---|---|---:|---:|---:|---:|---:|---:|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"
    }
  }
  ...
  /^METRICS / {
    ...
    pc_cmd_catchup_events = metric("pc_cmd_catchup_events")
    pc_cmd_catchup_extra = metric("pc_cmd_catchup_extra")
    pc_executor_threads = metric("pc_executor_threads")
    ...
    if (format == "csv") {
      printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n",
        ...
        duplicate, inflight, qos_incompatibility
    } else {
