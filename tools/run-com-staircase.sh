...
record_stage_metrics() {
  ...
  local pc_config pc_executor_threads
  ...
  pc_config="$(grep -F '[com-perf] pc:' "$stage_log" 2>/dev/null |
    tail -1 || true)"
  pc_executor_threads="$(extract_key_value "$pc_config" executor_threads)"
